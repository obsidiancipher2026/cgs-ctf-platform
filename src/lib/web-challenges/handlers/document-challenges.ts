...
// 27 — IDOR Inbox
// ═══════════════════════════════════
const PROFILE_FLAG = 'CGS{1d0r_1s_num3r1c_bl1ndn3ss}'
const idorInboxUsers = [
  { id:1,  username:'player',     full_name:'Alex Morgan',       email:'alex.morgan@cgslabs.io',      bio:'Frontend developer. I build dashboards and argue about CSS frameworks.' },
  { id:2,  username:'jchen',      full_name:'Jessica Chen',      email:'jessica.chen@cgslabs.io',     bio:'Backend engineer. Go enthusiast. I drink too much cold brew.' },
  { id:3,  username:'mwilson',    full_name:'Marcus Wilson',     email:'marcus.wilson@cgslabs.io',    bio:'DevOps lead. If it runs, I probably deployed it.' },
  { id:4,  username:'sgarcia',    full_name:'Sofia Garcia',      email:'sofia.garcia@cgslabs.io',     bio:'Security analyst. I break things so you don\'t have to.' },
  { id:5,  username:'rkim',       full_name:'Ryan Kim',          email:'ryan.kim@cgslabs.io',         bio:'Data scientist. I turn CSVs into PowerPoint decks.' },
  { id:6,  username:'lpatel',     full_name:'Lena Patel',        email:'lena.patel@cgslabs.io',       bio:'Product manager. I write tickets no one reads.' },
  { id:7,  username:'tnguyen',    full_name:'Tyler Nguyen',      email:'tyler.nguyen@cgslabs.io',     bio:'Mobile developer. React Native is my frenemy.' },
  { id:8,  username:'abrown',     full_name:'Aisha Brown',       email:'aisha.brown@cgslabs.io',      bio:'UX designer. Pixels matter. Margins matter more.' },
  { id:9,  username:'clee',       full_name:'Chris Lee',         email:'chris.lee@cgslabs.io',        bio:'SRE on call 24/7. My pager is my best friend.' },
  { id:10, username:'jmartinez',  full_name:'Jordan Martinez',   email:'jordan.martinez@cgslabs.io',  bio:'QA engineer. I find bugs before users do.' },
  { id:11, username:'kthomas',    full_name:'Kira Thomas',       email:'kira.thomas@cgslabs.io',      bio:'Tech writer. I turn chaos into documentation.' },
  { id:12, username:'dclark',     full_name:'Derek Clark',       email:'derek.clark@cgslabs.io',      bio:'Platform engineer. Kubernetes whisperer.' },
  { id:13, username:'mhall',      full_name:'Maya Hall',         email:'maya.hall@cgslabs.io',        bio:'Intern. I ask too many questions and that is a good thing.' },
  { id:14, username:'vramirez',   full_name:'Victor Ramirez',    email:'victor.ramirez@cgslabs.io',   bio:'Database admin. I speak SQL and dream in schemas.' },
  { id:15, username:'nwright',    full_name:'Nora Wright',       email:'nora.wright@cgslabs.io',      bio:'Scrum master. I protect the team from surprise meetings.' },
  { id:16, username:'ogreen',     full_name:'Oscar Green',       email:'oscar.green@cgslabs.io',      bio:'Embedded systems. C is my first language. Assembly is my guilty pleasure.' },
  { id:17, username:'hlee',       full_name:'Hannah Lee',        email:'hannah.lee@cgslabs.io',       bio:'ML engineer. I train models and occasionally they listen.' },
  { id:18, username:'pwright',    full_name:'Patrick Wright',    email:'patrick.wright@cgslabs.io',   bio:'Cloud architect. AWS bill is my nemesis.' },
  { id:19, username:'zking',      full_name:'Zoe King',          email:'zoe.king@cgslabs.io',         bio:'Incident responder. Firefighter, but with keyboards.' },
...
const idorInboxHandler = (req: PlaygroundRequest): PlaygroundResponse => {
  ...
  if (req.path === '/profile' || req.path.startsWith('/profile')) {
    ...
    if (idNum === 21) {
      return serve('/', idorInboxPage(idB64, { id: 21, full_name: 'CLASSIFIED', email: 'flag@cgslabs.io', bio: PROFILE_FLAG }))
    }
    const user = idorInboxUsers.find(u => u.id === idNum)
    if (user) return serve('/', idorInboxPage(idB64, user))
    return serve('/', idorInboxNotFound())
  }
  ...
  if (req.path === '/api/profile') {
    ...
    let idNum: number
    try { idNum = parseInt(Buffer.from(idB64, 'base64').toString(), 10) } catch { idNum = NaN }
    if (idNum === 21) return json({ id: 21, full_name: 'CLASSIFIED', email: 'flag@cgslabs.io', bio: PROFILE_FLAG })
    const user = idorInboxUsers.find(u => u.id === idNum)
...
function idorInboxLoginPage(): string {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>CGS Inbox</title>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#0B1120;color:#E2E8F0;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:40px}
.login-wrapper{width:100%;max-width:440px}
.login-header{text-align:center;margin-bottom:32px}
.login-header .logo{width:56px;height:56px;background:linear-gradient(135deg,#3B82F6,#8B5CF6);border-radius:14px;display:inline-flex;align-items:center;justify-content:center;font-size:24px;font-weight:700;color:#fff;margin-bottom:16px;box-shadow:0 4px 20px rgba(59,130,246,0.3)}
.login-header h1{font-size:22px;font-weight:600;color:#F1F5F9;letter-spacing:-0.3px}
.login-header p{color:#64748B;font-size:14px;margin-top:6px}
.login-card{background:#111827;border:1px solid #1F2937;border-radius:12px;padding:32px;box-shadow:0 4px 24px rgba(0,0,0,0.4)}
label{display:block;font-size:13px;color:#94A3B8;margin-bottom:6px;font-weight:500}
input{width:100%;padding:11px 14px;background:#0B1120;border:1px solid #1F2937;border-radius:8px;color:#E2E8F0;font-size:14px;transition:border-color 0.2s;margin-bottom:16px;outline:none}
input:focus{border-color:#3B82F6}
button{width:100%;padding:12px;background:linear-gradient(135deg,#3B82F6,#8B5CF6);border:none;color:#fff;border-radius:8px;font-weight:600;font-size:14px;cursor:pointer;transition:opacity 0.2s}
button:hover{opacity:0.9}
.error{color:#EF4444;font-size:13px;margin-top:8px;text-align:center;display:none}
.creds{margin-top:20px;background:#111827;border:1px solid #1F2937;border-radius:8px;padding:14px 16px}
.creds h4{font-size:11px;color:#64748B;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px}
.creds .row{display:flex;justify-content:space-between;font-size:13px;padding:3px 0}
.creds .row .lbl{color:#64748B}.creds .row .val{color:#94A3B8;font-family:monospace;font-weight:500}
...
<div class="creds"><h4>Demo Credentials</h4>
<div class="row"><span class="lbl">Username</span><span class="val">player</span></div>
<div class="row"><span class="lbl">Password</span><span class="val">player123</span></div>
</div>
</div>
...
function idorInboxPage(idB64: string, user: { id: number; full_name: string; email: string; bio: string }): string {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Profile - ${user.full_name}</title>
...
.avatar{width:72px;height:72px;background:linear-gradient(135deg,#3B82F6,#8B5CF6);border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:700;color:#fff;flex-shrink:0}
.profile-top h2{font-size:22px;font-weight:600;color:#F1F5F9}
.profile-top .email{color:#64748B;font-size:14px;margin-top:4px}
.field{margin-bottom:16px}
.field-label{font-size:11px;color:#64748B;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;font-weight:500}
.field-value{font-size:14px;color:#CBD5E1;line-height:1.6}
.divider{border:none;border-top:1px solid #1F2937;margin:20px 0}
.id-badge{display:inline-flex;align-items:center;gap:6px;background:#0B1120;border:1px solid #1F2937;border-radius:6px;padding:6px 12px;font-size:12px;color:#94A3B8;font-family:monospace}
.id-badge span{color:#3B82F6;font-weight:600}
.url-bar{background:#0B1120;border:1px solid #1F2937;border-radius:8px;padding:10px 14px;margin-top:20px;font-family:monospace;font-size:12px;color:#64748B;word-break:break-all}
...
<div><h2>${user.full_name}</h2><div class="email">${user.email}</div></div>
</div>
<hr class="divider">
<div class="field"><div class="field-label">Bio</div><div class="field-value">${user.bio}</div></div>
<hr class="divider">
<div class="field"><div class="field-label">Employee ID</div><div class="id-badge">ID <span>${user.id}</span></div></div>
<div class="url-bar"><em>/profile?id=${idB64}</em></div>
</div>
</div>
...
function idorInboxNotFound(): string {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>User Not Found</title>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#0B1120;color:#E2E8F0;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:40px}
.card{background:#111827;border:1px solid #1F2937;border-radius:12px;padding:48px;text-align:center;max-width:440px;width:100%}
...
export const documentChallenges: ChallengeDef[] = [
  ...
  { slug: 'idor-inbox',         title: 'IDOR Inbox',         handler: idorInboxHandler },
]
